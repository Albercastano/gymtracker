...
function recordSet(reps,weight){
 ...
 A.sets.push({id:id(),sessionId:A.session.id,exerciseId:e.id,setNumber:set,reps,weight,result:"normal",order:A.idx,mode:it.mode});
...
function endWorkout(){A.session.end=now();S.sessions.push(A.session);S.sets.push(...A.sets);S.plan[today()]={date:today(),type:"done",routineId:A.session.routineId,status:"done"};save();clearActiveWorkout();let summary=coachSummary(A.session.id);A=null;GT.show("home");GT.render();setTimeout(()=>alert(summary),100)}
...
function renderWorkout(){let it=currentItem(),e=ex(it.exerciseId),set=currentSet();let target=it.mode==="failure"?"F":(it.repsPlan[set-1]||it.repsPlan[0]);let label=it.mode==="time"?"Segundos":"Reps";document.getElementById("workout").innerHTML=`<div class="card focus"><div><div class="muted">${rt(A.session.routineId).name}</div><div class="focusTitle">${e.name}</div><div class="timeBadge">${it.mode==="time"?"Ejercicio por tiempo":it.mode==="failure"?"Al fallo":"Repeticiones"}</div></div><div class="focusStats"><div class="focusStat"><span>Serie</span><b>${set}/${it.sets}</b></div><div class="focusStat"><span>${label}</span><b>${target}</b></div><div class="focusStat"><span>Peso</span><b>${it.mode==="time"?"—":(it.weight||0)}</b><small>${it.mode==="time"?"":"kg"}</small></div></div><button class="wall primary bigAction ${it.mode==="time"?"timeAction":""}" onclick="finishSet()">${it.mode==="time"?"Iniciar crono":"Fin serie"}</button></div>`}

function resumeActiveWorkout(){let old=getActiveWorkout();if(!old)return false;A=old;GT.show("workout");renderWorkout();return true}
