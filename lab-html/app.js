const GT={
 ...
 show(id){
  ...
  document.getElementById(id).classList.add("on");if(id==="workout" && !A && getActiveWorkout()){resumeActiveWorkout();return;}
 ...
 renderHome(){
  ...
  document.getElementById("home").innerHTML=`${active?`<div class="card resumeBanner activeWorkoutBanner"><h2>Entrenamiento en curso</h2><p class="muted">Se encontró una sesión sin terminar.</p><div class="row"><button class="wall primary" onclick="resumeActiveWorkout()">Continuar</button><button class="danger" onclick="discardActiveWorkout()">Descartar</button></div></div>`:""}<div class="card alphaNotice"><h2>Alpha 1.0 estable</h2><p class="safeText">Antes de limpiar caché o cambiar de móvil: Ajustes → Backup antes de actualizar.</p></div><div class="card profileCard"><h2>Peso corporal</h2><div class="weightBig">${S.profile?.bodyWeight||"—"} <small>kg</small></div><p class="muted">Objetivo: ${S.profile?.targetWeight||"—"} kg · Último: ${(S.weights||[]).at(-1)?.date||"sin datos"}</p><div class="weightSaveRow"><input id="homeBodyWeight" type="number" step=".1" placeholder="Peso corporal kg" value="${S.profile?.bodyWeight||""}"><button class="wall primary saveWeightBtn" onclick="saveBodyWeightFromHome()">Guardar peso</button></div></div><div class="card hero"><div class="muted">Buenos días ${currentUser}</div><h1>${done?"Hoy ya has entrenado":r?"Hoy toca":"Sin rutina"}</h1><p class="muted">${done?"Buen trabajo. Próximo: descanso o siguiente plan.":r?r.name:"Importa o crea una rutina para empezar."}</p>${r?`<div class="routinePreview">${r.items.map(i=>`<div>${ex(i.exerciseId)?.name||"Ejercicio"} · ${i.sets} series · ${i.mode==="time"?(i.repsPlan[0]+"s"):i.mode==="failure"?"fallo":(i.repsPlan[0]+" reps")}</div>`).join("")}</div><button class="wall primary bigAction" onclick="startRoutine('${r.id}')">${done?"Repetir entreno":"Comenzar"}</button>`:`<button class="wall primary" onclick="GT.show('routines')">Ir a rutinas</button>`}</div>`;
 },
 renderRoutines(){
  let el=document.getElementById("routines");if(!el)return;
  el.innerHTML=`<div class="card routineGuide">
  <h2>Guía rápida para construir tu rutina</h2>
  <div class="guideSteps">
    <div class="guideStep"><b>1. Pon nombre</b>Ejemplo: Torso A, Pierna B, Full Body o Calistenia.</div>
    <div class="guideStep"><b>2. Añade ejercicios</b>Busca en la mochila o crea uno nuevo. Usa los ejercicios que realmente haces.</div>
    <div class="guideStep"><b>3. Define el objetivo</b>Series, reps/tiempo/fallo, peso inicial y descanso.</div>
    <div class="guideStep"><b>4. Ordena y guarda</b>Usa ↑ ↓, duplica si hace falta y asigna la rutina a un día.</div>
  </div>
  <h3>Formato para importar</h3>
  <div class="guideExample">Press banca 4x8 80kg descanso 180
Dominadas 4x fallo descanso 180
L-Sit 4x20s descanso 90</div>
</div><div class="card importBox"><div class="builderHeader"><h2>Constructor PRO</h2><span class="pill">Lab 8.0</span></div><div class="helpBox">Importa, crea y edita sin ventanas emergentes. Los cambios se guardan solos.</div><pre>Press banca 4x8 80kg descanso 180
Dominadas 4x fallo descanso 180
L-Sit 4x20s descanso 90</pre><input id="importName" placeholder="Nombre de rutina"><textarea id="importText" placeholder="Pega aquí la rutina"></textarea><div class="row"><button class="wall primary" onclick="importRoutine()">Importar rutina</button><button onclick="exampleImport()">Ejemplo</button></div></div>
 ...
 renderSettings(){
  ...
  <div class="card"><h2>Configuración PRO</h2><div class="profileGrid"><label>Tema<select onchange="S.settings.theme=this.value;save();GT.render()"><option value="luxury" ${S.settings.theme==='luxury'?'selected':''}>Luxury</option><option value="elegance" ${S.settings.theme==='elegance'?'selected':''}>Elegance</option><option value="carbon" ${S.settings.theme==='carbon'?'selected':''}>Carbon</option></select></label><label>Descanso<input type="number" value="${S.settings.rest||120}" onchange="S.settings.rest=+this.value;save()"></label><label>Incremento<input type="number" step=".25" value="${S.settings.inc||2.5}" onchange="S.settings.inc=+this.value;save()"></label><label>Tiempo por defecto<input type="number" value="${S.settings.defaultTime||20}" onchange="S.settings.defaultTime=+this.value;save()"></label><label>Sonido<select onchange="S.settings.sound=this.value==='true';save()"><option value="false" ${!S.settings.sound?"selected":""}>No</option><option value="true" ${S.settings.sound?"selected":""}>Sí</option></select></label><label>Vibración<select onchange="S.settings.vibration=this.value==='true';save()"><option value="false" ${!S.settings.vibration?"selected":""}>No</option><option value="true" ${S.settings.vibration?"selected":""}>Sí</option></select></label></div></div>
