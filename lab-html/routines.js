function cleanExerciseName(line){
 return (line||"").replace(/^\s*[-•*]\s*/,"").replace(/\b\d+\s*x\s*(?:al\s*)?fallo\b/ig,"").replace(/\b\d+\s*x\s*\d+\s*(?:[-\/]\s*\d+)?\b/ig,"").replace(/\b\d+\s*x\s*\d+\s*s\b/ig,"").replace(/\b\d+(?:[.,]\d+)?\s*kg\b/ig,"").replace(/\b(?:descanso|rest)\s*\d+\s*s?\b/ig,"").replace(/\([^)]*\)/g,"").replace(/\s+/g," ").trim()
}
...
function parseLine(raw){
 ...
 let sets=0,repsPlan=[],mode="reps";let fail=raw.match(/(\d+)\s*x\s*(?:al\s*)?fallo/i),time=raw.match(/(\d+)\s*x\s*(\d+)\s*s\b/i),reps=raw.match(/(\d+)\s*x\s*(\d+)(?:\s*[-\/]\s*(\d+))?/i);
 ...
 let mw=raw.match(/(\d+(?:[.,]\d+)?)\s*kg/i),mr=raw.match(/(?:descanso|rest)\s*(\d+)\s*s?/i);
...
function importRoutine(){
 let name=document.getElementById("importName").value.trim()||"Rutina importada";let txt=document.getElementById("importText").value;let items=[];
 ...
 S.routines.push({id:id(),name,tag:"Importada",items});save();GT.render();alert("Rutina importada: "+name)
}
function exampleImport(){document.getElementById("importName").value="Torso A";document.getElementById("importText").value=`Press banca 4x8 80kg descanso 180
Dominadas 4x fallo descanso 180
Remo barra 4x10-12 60kg descanso 120
Fondos en paralelas 4x al fallo descanso 150
...
function addExercise(rid){let r=rt(rid);let name=prompt("Ejercicio");if(!name)return;let sets=+(prompt("Series","4")||4),reps=+(prompt("Reps","8")||8),weight=+(prompt("Peso kg","0")||0),rest=+(prompt("Descanso segundos",String(S.settings.rest||120))||120);let e=getExercise(name);r.items.push({id:id(),exerciseId:e.id,sets,repsPlan:Array.from({length:sets},()=>reps),weight,rest,mode:"reps",order:r.items.length+1});save();GT.render()}
function delItem(rid,iid){let r=rt(rid);r.items=r.items.filter(i=>i.id!==iid).map((x,i)=>({...x,order:i+1}));save();GT.render()}
function updItem(rid,iid,k,v){let it=rt(rid).items.find(i=>i.id===iid);if(k==="sets"){it.sets=+v;it.repsPlan=Array.from({length:it.sets},(_,i)=>it.repsPlan[i]||8)}else if(k==="reps")it.repsPlan=Array.from({length:it.sets},()=>+v);else if(k==="weight")it.weight=+v;else if(k==="rest")it.rest=+v;save()}
