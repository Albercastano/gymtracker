#!/usr/bin/env python3
"""Inline Chromium harness for Phoenix Build 019.
...
def build_html() -> str:
    ...
    html = re.sub(r'<link rel="stylesheet" href="styles\.css\?v=019">', f"<style>{css}</style>", html)
    html = re.sub(
        r'<link rel="stylesheet" href="themes/apex/apex\.css\?v=019" data-phx-apex-bootstrap>',
...
def boot_snapshot(page) -> dict:
    return page.evaluate(
        """()=>({
          ...
          shapeRenderReady:document.documentElement.dataset.phxShapeRenderReady,
          rendererVersion:window.PhoenixForgeHomeRenderer?.version,
          homeRendered:document.getElementById("home")?.dataset?.forgeRendered,
        ...
        """async()=>{
          function visualSignature(root){
            const walk=node=>{
              ...
              return `${node.tagName.toLowerCase()}${cls?'.'+cls:''}[${children}]`;
            };
            return walk(root);
          }
          const declarativeRoot=document.querySelector('#home > .home-phoenix');
          const declarativeSignature=visualSignature(declarativeRoot);
          const declarativeText=declarativeRoot?.innerText;
          const declarativeActions=[...document.querySelectorAll('#home [data-forge-action]')].map(el=>el.dataset.forgeAction);
          App.renderHomeLegacy(false);
          await new Promise(resolve=>requestAnimationFrame(()=>requestAnimationFrame(resolve)));
          const legacyRoot=document.querySelector('#home > .home-phoenix');
          const legacySignature=visualSignature(legacyRoot);
          const legacyText=legacyRoot?.innerText;
          App.renderHome(false);
          const restoredDeclarative=document.getElementById('home')?.dataset?.forgeRendered==='true';
          ...
          const dataButton=document.querySelector('#home [data-forge-action="open-data"]');
          dataButton?.click();
          ...
          const report=PhoenixForgeDiagnostics.run(App);
          return {
            visualEquivalence:declarativeSignature===legacySignature,
            textEquivalence:declarativeText===legacyText,
            declarativeActions,
            restoredDeclarative,
            rendererReport:PhoenixShapeEngine.lastRender,
            ...
            diagnostics:{homeRendered:report.homeRendered,renderReady:report.renderReady,renderer:report.renderer},
            forgeLab:{
              ...
              message:document.getElementById('forgeShapeMessage')?.textContent
            }
          }
        }"""
    )
...
def main() -> int:
    ...
    all_booted = all(
        ...
        and item["boot"]["shapeRenderReady"] == "true"
        and item["boot"]["homeRendered"] == "true"
        and item["boot"]["rendererVersion"] == "0.2.0"
    ...
    output["passed"] = bool(
        all_booted
        and functional.get("visualEquivalence")
        and functional.get("textEquivalence")
        and functional.get("restoredDeclarative")
        and functional.get("diagnostics", {}).get("homeRendered")
